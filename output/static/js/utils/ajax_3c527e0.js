define('ops:static/js/utils/ajax.js', function(require, exports, module) {
  ...
  function Ajax(options) {
      ...
      $private.xhrConnection = function xhrConnection(type, url, data, options) {
          ...
          // custom get function
          if (type === 'get' && $private.isObject(data)) {
              var divider = '?';
              if (url.search(/\?/) >= 0) {
                  divider = '&';
              }
              url = [url, $private.objectToQueryString(data)].join(divider)
          }
          // custom get function end
  
          xhr.open(type, url || '', true)
          $private.setHeaders(xhr, options.headers)
          xhr.addEventListener("progress", $private.progress, false);
      ...
      $private.progress = function progress(event) {
          var xhr = this
          if (event.lengthComputable) {
              var percentComplete = event.loaded / event.total;
          } else {
              var percentComplete = xhr.readyState === xhr.DONE ? 1 : 0
          }
          $private.methods.progress
              .call($private.methods, percentComplete)
      }
